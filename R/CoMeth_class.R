...
#' @param methylation_type A character vector storing the type of methylation loci for these m-tuples. Possible mcols are "CG", "CHG", "CHH" or "CNN". Multiple mcols can be specified, e.g. c("CG", "CHG"). Only a single value is accepted, and not a list, because \code{methylation_type} must be the same for all samples in a \code{CoMeth} object.
...
CoMeth <- function(sample_names, seqnames, pos, counts, strand = NULL, m, methylation_type, seqinfo, sort_cometh = TRUE){
  ...
  ## Check 'sample_names' match column names of 'pos' and 'counts'
  ...
  if (m > 2){
    ...
    gr <- GRanges(seqnames = combined_data[['seqnames']], ranges = IRanges(start = combined_data[['pos']][[1]], end = combined_data[['pos']][[m]]), strand = combined_data[['strand']], seqinfo = seqinfo, DataFrame(combined_data[['pos']])[, -c(1, m), drop = FALSE])
    } else{
...
validCoMeth <- function(object){
  ...
  ## Check that "extra positions" are stored as metadata of rowData if m > 2
  if (m > 2){
    if (paste0('pos', seq(from = 2, to = m - 1, by = 1), sep = '') %in% colnames(mcols(rowData(object)))){
      msg <- validMsg(msg, paste0("'m' is greater than 2 but '", paste0('pos', seq(from = 2, to = m - 1, by = 1), sep = ''), "' not stored as metadata of rowData"))
    }
  }
